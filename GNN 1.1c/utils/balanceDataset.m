...
if (size(dataSet.trainSet.targets,1)>1)
    ...
    indexsupervised=find(sum(dataSet.trainSet.maskMatrix>0)+sum(dataSet.trainSet.maskMatrix'>0));
    indexpos=intersect(find(dataSet.trainSet.targets==1),indexsupervised);
    indexneg=intersect(find(dataSet.trainSet.targets==-1),indexsupervised);
    ...
    % validationSet
    if isfield(dataSet,'validationSet')
        indexsupervised=find(sum(dataSet.validationSet.maskMatrix>0)+sum(dataSet.validationSet.maskMatrix'>0));
        indexpos=intersect(find(dataSet.validationSet.targets==1),indexsupervised);
        indexneg=intersect(find(dataSet.validationSet.targets==-1),indexsupervised);
        sizepos=size(indexpos,2);
        sizeneg=size(indexneg,2);
        if sizepos<sizeneg
